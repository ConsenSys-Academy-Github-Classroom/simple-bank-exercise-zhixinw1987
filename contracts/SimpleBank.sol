...
contract SimpleBank {
    ...
    modifier isEnrolled(address user){
      require(enrolled[user]== true,"User is not enrolled yet");
      _;
    }

    modifier notEnrolled(address user){
      require(enrolled[user]== false,"User is not enrolled yet");
      _;
    }
    ...
    function withdraw(uint withdrawAmount) public returns (uint) {
      ...
      //    sender's balance
		...
		require(sent, string(abi.encodePacked("Could not transfer back to owner:", data)));
